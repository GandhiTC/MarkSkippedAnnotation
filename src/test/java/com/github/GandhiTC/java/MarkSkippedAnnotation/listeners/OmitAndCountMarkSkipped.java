...
public class OmitAndCountMarkSkipped extends TestListenerAdapter implements IMethodInterceptor
{
	...
	{
		for(ITestNGMethod itngMethod : testContext.getAllTestMethods())
		{
			String itngMethodName = itngMethod.getConstructorOrMethod().getMethod().getName();

			if(disabledMethods.contains(itngMethodName))
			{
				TestResult newResult = new TestResult();
				newResult.setTestName(itngMethodName);
				newResult.setTestClass(itngMethod.getTestClass());
				newResult.setMethod(itngMethod);
				newResult.setStatus(3);

				testContext.getSkippedTests().addResult(newResult, itngMethod);
			}
		}
